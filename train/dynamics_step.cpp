...
static PyObject *py_update(PyObject *self, PyObject *args) {
  ...
  double h_max = 1.0 / (40.0 * fin_f); /* Same as ode45 MaxStep */
  ...
  for (int step = 0; step < n_steps; step++) {
    rk4_step(state, h, bp, fp, t_local, t_k, c_A, fin_f, A1_ref, A2_ref, A3_ref,
             A4_ref, alpha5_ref, A1_hist, A2_hist, A3_hist, A4_hist,
             alpha5_hist);
    t_local += h;
  }
  ...
  PyObject *result = Py_BuildValue(
      "(Oddddd)", state_out,
      cpg_A_val(A1_ref, A1_hist, c_A, dt_total),
      cpg_A_val(A2_ref, A2_hist, c_A, dt_total),
      cpg_A_val(A3_ref, A3_hist, c_A, dt_total),
      cpg_A_val(A4_ref, A4_hist, c_A, dt_total),
      cpg_A_val(alpha5_ref, alpha5_hist, c_A, dt_total));
  Py_DECREF(state_out);
  return result;
}
